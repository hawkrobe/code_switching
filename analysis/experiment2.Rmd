...
d.comprehension <- d.comprehension.raw %>%
  ...
  mutate(correct = response == tangram,
         logrt = log(rt),
         upper = mean(logrt) + 3*sd(logrt),
         lower = mean(logrt) - 3*sd(logrt))
```
...
```{r}
d.4afc %>%
  ggplot(aes(x = factor(correct), y = logrt)) +
    geom_boxplot()
...
d.4afc %>%
  filter(rt > 1000) %>%
  select(playerId, tangram, condition, targetAudience, correct, rt) %>%
  mutate(correct = 100 * correct) %>%
  gather(metric, value, correct, rt) %>%
  group_by(metric, condition, targetAudience) %>%
  tidyboot_mean(value, na.rm = T, nboot = 100) %>%
