...
post_test <- post_test.raw %>%
  ...
  select(gameId, playerId, tangram, utt_length_words, ownGroup) %>%
...
```
Try to visualize central tendencies.
...
post_test %>%
  ...
  ggplot(aes(x = blockOrder, y = log_length, color = ownGroup)) +
    geom_point(position = position_jitter(width = .2, height = 0.05), size = .1, alpha = 0.2) +
    geom_boxplot(aes(x = blockOrder, color = ownGroup, y = log_length),
                 outlier.shape = NA, alpha = 0.5, width = .5) +
...
post_test.raw %>%
  filter(gameId %in% full_post_test) %>%
  select(gameId, playerId, tangram, utt_length_words, ownGroup, text) %>%
  write_json(here('data/comprehension/post_test_utterances.json'))
```